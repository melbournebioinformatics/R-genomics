...
we want to think about a human genome rather than E. coli. Change `genome_length_mb` to 3000 and figure out the weight of the human genome. 
...
Instead of having to remember that 978Mb = 1 picogram we could create a function to convert genome lengths to weights. The R code below defines a function to do this job.  

```{r, purl=FALSE}
genome_weight_from_length <- function(genome_length_mb){
	genome_length_mb / 978
}
```

Once we create this function we can get a genome's weight if we have its length by typing the name of our function and then the length in brackets.

```{r,purl=FALSE}
	# For the human genome
genome_weight_from_length(3000)
```


### Challenge

Create a function to calculate genome lengths in Mb from genome weights in picograms.  Test that your function works by using it to calculate the length of the human genome given its weight `3.07` picograms.
...
These two vectors naturally fit together. We can formally associate the species names with their genome lengths using a named vector.
...
Many functions in R accept vectors as arguments and operate accordingly.  For example we could use the function we created earlier to calculate genome weights to calculate a vector of genome weights given a vector of genome lengths.

```{r, purl=FALSE}
genome_weight_from_length(glengths)
```
...
```{r,purl=FALSE}
genome_length_from_weight <- function(genome_weight_picograms){
	genome_weight_picograms * 978
}
