...
We are studying a population of Escherichia coli (designated Ara-3), which were propagated for more than 40,000 generations in a glucose-limited minimal medium. This medium was supplemented with citrate which E. coli cannot metabolize in the aerobic conditions of the experiment. Sequencing of the populations at regular time points reveals that spontaneous citrate-using mutants (Cit+) appeared at around 31,000 generations. This metadata describes information on the Ara-3 clones and what the columns represent:
...
The metadata file required for this lesson can be downloaded by clicking on this [link](./data/Ecoli_metadata.csv)
...
citrate <- metadata$cit
levels(citrate)
nlevels(citrate)
```

Note that we used the `$` operator to extract the `cit` column of the dataframe. We assigned this to a new object called `citrate` for convenience.
